...
specLayout1 = describe "layout1" $ do
  it "positions blocks simple in a simple linear layout" $ do
    ...
    let ([positionedBlock1, positionedBlock2, (PositionedFork _i _c [l3] [r3] _gCId x3 y3 _maxX _minY)], skewerMaxX, skewerMinY) = position' blocks 0.0 0.0
    let (x1, y1, maxX1, minY1) = getPosition positionedBlock1
    let (x2, y2, maxX2, minY2) = getPosition positionedBlock2
    let (x3l, y3l, maxX3l, minY3l) = getPosition l3
    let (x3r, y3r, maxX3r, minY3r) = getPosition r3
    ...
    skewerMaxX `shouldBe` 6.0
    skewerMinY `shouldBe` (defaultBoundingBoxHeight * (-4.0))
